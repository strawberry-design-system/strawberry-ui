...
import { paginationStyle, pageButtonStyle, pageTextStyle } from '@strawberry-ui/styles/components/Pagination.css'
		...
		<div className={paginationStyle}>
			...
			{Array.from({ length: total }, (_, index) => index + 1).map(page => {
				if (
					...
					(page <= boundaries && selected <= boundaries - siblings) ||
					(page > total - boundaries && selected > total - (boundaries - siblings))
				) {
					return (
						<button
